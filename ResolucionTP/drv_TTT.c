// Parto de drv3 para realizar las modificaciones a read y write para ver qué hace
...
static int __init drv_TTT_init(void) /* Constructor */
{
    ...
    if (IS_ERR(dev_ret = device_create(cl, NULL, first, NULL, "SdeC_drvTTT")))
    {
        class_destroy(cl);
        unregister_chrdev_region(first, 1);
    ...
    cdev_init(&c_dev, &pugs_fops); // inicializamos las operaciones a realizar
